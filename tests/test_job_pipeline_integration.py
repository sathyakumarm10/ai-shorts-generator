"""End-to-End integration test for background job execution of the full Shorts pipeline.

Submits a background job, monitors progress updates across actual pipeline stages,
waits for job completion, and validates the rendered 1080x1920 short videos.
"""
...
class TestJobPipelineRealIntegration:
    def test_background_job_pipeline_execution_to_completion(self, tmp_path: Path):
        ...
        source_video_path = tmp_path / "bg_pipeline_source_70s.mp4"
        cmd = [
            ffmpeg_bin,
            "-y",
            "-f",
        ...
        shorts_service = ShortsGenerationService(
        ...
        job_runner = JobRunnerService(job_service=job_service, shorts_service=shorts_service, max_workers=2)
        ...
        future = job_runner.submit_job(job.job_id, req)

        # 5. Wait for background execution to complete
        future.result(timeout=120.0)

        # 6. Verify final completed job record and generated artifacts
        completed_job = job_service.get_job(job.job_id)
        assert completed_job is not None
        assert completed_job.error is None, f"Job failed with error: {completed_job.error}"
        ...
        # 7. Validate output metadata with ffprobe
        meta = metadata_service.extract_metadata(final_video_path)
